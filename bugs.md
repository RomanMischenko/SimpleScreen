...
### [x] 2. Дефолтные хоткеи Cmd+Shift+3 / Cmd+Shift+4 совпадают с системными скриншотами macOS
...
### [x] 8. `DateFormatter` без явной локали — нестабильные имена файлов
...
**Status:** исправлено в 2606c6d — `DateFormatter` в `saveImageToDisk` получил фиксированную локаль `en_US_POSIX`, имена файлов теперь стабильны независимо от календаря/цифр системной локали.

### [WONTFIX] 9. NotificationCenter спамится уникальными идентификаторами
...
**Status:** by design — уникальные UUID-идентификаторы намеренны. Единый переиспользуемый identifier заставил бы новое уведомление затирать предыдущее; для баннеров об ошибках (fallback на Desktop, провал сохранения, слишком маленькое выделение) и подтверждений захвата важно, чтобы повторные события накапливались отдельными записями, а не молча замещали друг друга.

### [x] 10. `requestAuthorization` показывает диалог на каждом старте, если ранее не определились
...
**Status:** исправлено в fde8c22 — убран безусловный `requestAuthorization()` со старта; разрешение запрашивается лениво через приватный `post(_:)` в `NotificationManager`, который проверяет `getNotificationSettings` и спрашивает систему только при первом реальном уведомлении (`.notDetermined`), молча пропуская `.denied`.

### [x] 11. Кастомная горячая клавиша может оказаться без модификаторов или быть только модификатором
**Файл:** `Preferences/PreferencesView.swift:151-162`
**Status:** исправлено в 5b6dfde — монитор записи теперь отклоняет нажатия без модификаторов и «голые» модификаторы (beep, запись продолжается); регистрируется только комбинация модификатор(ы) + обычная клавиша.
...
### [x] 12. `attemptRegister` теряет хоткей, если старый был `nil`
**Файл:** `Preferences/PreferencesView.swift:165-194`
**Status:** исправлено в a534f92 — перерегистрация сделана атомарной: новая комбинация сначала проверяется probe-регистрацией под временным id (`id + 1000`), а «живой» хоткей сносится только после её успеха. При конфликте старая регистрация (или её отсутствие) остаётся нетронутой, плюс добавлен guard на повторную запись той же комбинации.
