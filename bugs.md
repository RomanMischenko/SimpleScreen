...
**Status:** исправлен «худший случай» — когда `RegisterEventHotKey` возвращает `eventHotKeyExistsErr`, `AppDelegate` больше не глотает ошибку: `NotificationManager.postHotkeyConflictNotification` шлёт баннер с фиксированным identifier `ss.hotkey.conflict`, а `StatusBarController` помечает соответствующий пункт меню суффиксом `(in use by system)` и сбрасывает `keyEquivalent`. `PreferencesView` получает начальные флаги конфликта и сразу показывает красную плашку; при успешной перенастройке через `onResolveConflict` индикатор в меню гаснет. `settings.*Shortcut` в катче больше не зануляется. «Лучший случай» (коэкзистенция с системным шорткатом, двойной скриншот) — by design: Carbon регистрирует хоткей успешно, наш скриншот работает, отдельной обработки не требует.
